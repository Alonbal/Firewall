...
int init_rule_table(void)   //returns 0 for success, 1 otherwise
{
    ...
    //create sysfs device
    rules_device = device_create(fw_sysfs_class, NULL, MKDEV(major_number, MINOR_RULES), NULL, DEVICE_NAME_RULES);	
    ...
    {
        device_destroy(fw_sysfs_class, MKDEV(major_number, MINOR_RULES));
        unregister_chrdev(major_number, DEVICE_NAME_RULES);
        return 1;
    }

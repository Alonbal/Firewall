...
void print_reason(int reason)
{
    ...
    case -3:
        printf("REASON_PART_OF_CONNECTION\t\t");
        break;
...
int main(int argc, char *argv[]) 
{
    ...
    case 2:
        if (!strcmp(argv[1], "show_rules")) 
        {
            ...
            path = "/sys/class/fw/rules/rules";
            device = fopen(path, "r");
            if (!device) error_occured("fopen rules")
        ...
        else if (!strcmp(argv[1], "show_log")) 
        {
            ...
            if (!device) error_occured("popen log")
        ...
        {
            //showing connection table
            path = "/sys/class/fw/conns/conns";
            device = fopen(path, "r");
            if (!device) error_occured("fopen conns")
            unsigned int src_ip, dst_ip;
            unsigned short src_port, dst_port, state;

            printf("Source IP\t\tDest. IP\t\tSource Port\tDest. Port\tState\n");
            
            while (getline(&line, &len, device) != -1)
            {
                //printing file
                if (sscanf(line, "%u %u %hu %hu %hu", &src_ip, &dst_ip, &src_port, &dst_port, 
                    &state) != 5) error_fclose_file(device, "reading from conns failed")
                print_ip(src_ip);
                printf("\t\t");
                print_ip(dst_ip);
                printf("\t\t");
                printf("%hu\t\t%hu\t\t", ntohs(src_port), ntohs(dst_port));
                print_state(state);
                printf("\n");
            }
            fclose(device);
    ...
    default:
        error_occured("wrong amount of inputs")
        break;
    }

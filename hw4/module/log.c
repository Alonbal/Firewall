...
static struct klist_iter *show_iter = NULL;        //for list iterations
static struct klist_node *curr_node = NULL;        //for list iterations
    ...
    while ((curr_node = klist_next(&iter)))
    {
        ...
        if ((src_ip == entry->row.src_ip) && (dst_ip == entry->row.dst_ip) && 
            (src_port == entry->row.src_port) && (dst_port == entry->row.dst_port) &&
                (protocol == entry->row.protocol) && (entry->row.action == verdict))      //same connection and same action: update
        {
...
int init_log_module(void)
{
    ...
    //create sysfs device
    reset_device = device_create(fw_class, NULL, MKDEV(reset_major, 0), NULL, DEVICE_NAME_LOG);	
    ...
    {
        device_destroy(fw_class, MKDEV(reset_major, 0));
        unregister_chrdev(reset_major, DEVICE_NAME_LOG);
        return 1;
    }
    ...
    //create /dev device
    log_device = device_create(fw_class, NULL, MKDEV(log_major, 0), NULL, CLASS_NAME "_" DEVICE_NAME_LOG);	
...
{
    klist_clean();
    device_destroy(fw_class, MKDEV(log_major, 0));
    unregister_chrdev(log_major, CLASS_NAME "_" DEVICE_NAME_LOG);
    device_remove_file(reset_device, (const struct device_attribute *)&dev_attr_reset.attr);
    device_destroy(fw_class, MKDEV(reset_major, 0));
    unregister_chrdev(reset_major, DEVICE_NAME_LOG);

...
mitm.c:
Packets which are part of HTTP or FTP protocols are redirected to user space programs retrieving relevant data.
...
Compile it and run to allow http connection going through the firewall.
Retrieves the data, and does the following:
- Erases "gzip" from "Accept-Encoding" header in request (if appears)
...
Compile it and run to allow ftp connection going through the firewall.
